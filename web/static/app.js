...
let splatPollingInterval = null;
let splatStartedAt = 0;

async function initSplatCapture() {
    // Check if a capture is already in progress on page load
    try {
        const response = await fetch('/api/splat/status');
        const data = await response.json();
        console.log('Splat init status:', data.state, 'count:', data.count);
        if (data.state === 'capturing' || data.state === 'zipping') {
            // Resume UI for active capture
            document.getElementById('btn-splat-start').disabled = true;
            document.getElementById('btn-splat-stop').disabled = false;
            updateSplatUI(data);
            startSplatPolling();
        } else if (data.state === 'complete' && data.zipReady) {
            updateSplatUI(data);
        }
    } catch (error) {
        console.error('Error checking splat status on init:', error);
    }
    // Load previous sessions
    loadSplatSessions();
}

async function startSplatCapture() {
    const btnStart = document.getElementById('btn-splat-start');
    const btnStop = document.getElementById('btn-splat-stop');
    const statusBox = document.getElementById('splat-status');

    btnStart.disabled = true;
    btnStop.disabled = false;
    statusBox.style.display = 'block';
    document.getElementById('splat-state').textContent = 'Starting...';
    document.getElementById('splat-state').className = 'splat-state';
    document.getElementById('splat-count').textContent = '';
    document.getElementById('splat-download').style.display = 'none';

    try {
        const response = await fetch('/api/splat/start', { method: 'POST' });
        const data = await response.json();
        console.log('Splat start response:', response.status, data);

        if (data.success) {
            showNotification('Splat capture started', 'success');
            splatStartedAt = Date.now();
        } else if (data.message && data.message.indexOf('already') >= 0) {
            // Already in progress — treat as success
            showNotification('Splat capture already running', 'success');
        } else {
            showNotification(data.message || 'Failed to start', 'error');
            btnStart.disabled = false;
            btnStop.disabled = true;
            document.getElementById('splat-state').textContent = data.message || 'Failed to start';
            document.getElementById('splat-state').className = 'splat-state splat-error';
            return;
        }
        // Always start polling after a successful or already-running response
        startSplatPolling();
    } catch (error) {
        console.error('Error starting splat capture:', error);
        showNotification('Failed to start splat capture', 'error');
        btnStart.disabled = false;
        btnStop.disabled = true;
        document.getElementById('splat-state').textContent = 'Error: ' + error.message;
        document.getElementById('splat-state').className = 'splat-state splat-error';
    }
}

async function stopSplatCapture() {
    const btnStart = document.getElementById('btn-splat-start');
    const btnStop = document.getElementById('btn-splat-stop');

    btnStop.disabled = true;
    document.getElementById('splat-state').textContent = 'Stopping...';
    document.getElementById('splat-state').className = 'splat-state splat-zipping';

    try {
        const response = await fetch('/api/splat/stop', { method: 'POST' });
        const data = await response.json();
        console.log('Splat stop response:', response.status, data);

        if (data.success) {
            showNotification('Splat capture stopped, creating ZIP...', 'success');
        } else {
            showNotification(data.message || 'Stop command sent', 'error');
        }
        // Always keep polling to track zip progress and completion
        if (!splatPollingInterval) {
            startSplatPolling();
        }
    } catch (error) {
        console.error('Error stopping splat capture:', error);
        showNotification('Failed to stop splat capture', 'error');
        // Still try polling in case stop actually worked
        if (!splatPollingInterval) {
            startSplatPolling();
        }
    }
...
function startSplatPolling() {
    if (splatPollingInterval) clearInterval(splatPollingInterval);
    splatPollingInterval = setInterval(pollSplatStatus, 1500);
    // Also poll immediately
    pollSplatStatus();
}

async function pollSplatStatus() {
    try {
        const response = await fetch('/api/splat/status');
        const data = await response.json();
        console.log('Splat poll:', data.state, 'count:', data.count);
        updateSplatUI(data);

        // Stop polling on terminal states, but give grace period after start
        // (HD2K NEURAL engine optimization can take several minutes on first run)
        var isTerminal = (data.state === 'complete' || data.state === 'error');
        var graceExpired = (Date.now() - splatStartedAt) > 600000;
        if (data.state === 'idle' && graceExpired) {
            isTerminal = true;
        }
        // Never stop polling during initializing (engine optimizing)
        if (data.state === 'initializing') {
            isTerminal = false;
        }
        if (isTerminal) {
            clearInterval(splatPollingInterval);
            splatPollingInterval = null;
            splatStartedAt = 0;
            document.getElementById('btn-splat-start').disabled = false;
            document.getElementById('btn-splat-stop').disabled = true;
...
function updateSplatUI(data) {
    const statusBox = document.getElementById('splat-status');
    const stateEl = document.getElementById('splat-state');
    const countEl = document.getElementById('splat-count');
    const downloadDiv = document.getElementById('splat-download');
    const downloadLink = document.getElementById('splat-download-link');

    statusBox.style.display = 'block';

    if (data.state === 'initializing') {
        stateEl.textContent = 'Initializing HD2K NEURAL...';
        stateEl.className = 'splat-state splat-initializing';
        countEl.textContent = 'TensorRT engine may be optimizing, please wait';
    } else if (data.state === 'capturing') {
        stateEl.textContent = 'Capturing...';
        stateEl.className = 'splat-state splat-active';
        countEl.textContent = data.count + ' images captured';
    } else if (data.state === 'zipping') {
        stateEl.textContent = 'Creating ZIP...';
        stateEl.className = 'splat-state splat-zipping';
        countEl.textContent = data.count + ' images';
    } else if (data.state === 'complete') {
        stateEl.textContent = 'Complete';
        stateEl.className = 'splat-state splat-complete';
        countEl.textContent = data.count + ' images captured';
        if (data.zipReady && data.zipFile) {
            downloadDiv.style.display = 'block';
            downloadLink.href = '/api/splat/download/' + data.zipFile;
            downloadLink.textContent = 'Download ' + data.zipFile;
        }
    } else if (data.state === 'error') {
        stateEl.textContent = 'Error';
        stateEl.className = 'splat-state splat-error';
    } else if (data.state === 'idle') {
        statusBox.style.display = 'none';
    }
}

async function loadSplatSessions() {
    try {
        const response = await fetch('/api/splat/sessions');
        const data = await response.json();
        const container = document.getElementById('splat-sessions');
        ...
        container.innerHTML = '<h3 class="splat-sessions-title">Splat Capture Sessions</h3>' +
            data.sessions.map(function(s) {
                return '<div class="splat-session-item">' +
                    '<span class="splat-session-name">' + s.name + '</span>' +
                    '<span class="splat-session-meta">' + s.sizeFormatted + ' | ' + s.modifiedFormatted + '</span>' +
                    '<div class="splat-session-actions">' +
                    '<a href="/api/splat/download/' + s.name + '" class="btn btn-small btn-download" download>Download</a>' +
                    '<button class="btn btn-small btn-delete" onclick="deleteSplatSession(\'' + s.name + '\')">Delete</button>' +
                    '</div></div>';
            }).join('');
    } catch (error) {
        console.error('Error loading splat sessions:', error);
    }
}

async function deleteSplatSession(filename) {
    if (!confirm('Delete ' + filename + '?')) return;

    try {
        const response = await fetch('/api/splat/' + filename, { method: 'DELETE' });
        const data = await response.json();
        if (data.success) {
            showNotification('Session deleted', 'success');
            loadSplatSessions();
            // Re-poll status to clear the Complete/download UI
            pollSplatStatus();
        } else {
