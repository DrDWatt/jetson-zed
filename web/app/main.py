...
SPLAT_STATUS_FILE = os.path.join(VIDEO_DIR, ".splat_status")
...
async def index(request: Request):
    ...
    try:
        ...
        # Starlette < 0.28 (old API)
        return templates.TemplateResponse(
            "index.html",
            {"request": request, "status": status, "videos": videos}
        )
...
@app.post("/api/splat/start")
async def startSplatCapture():
    """Start Gaussian Splat still image capture"""
    status = getStatus()
    if status["state"] == "recording":
        return JSONResponse(
            status_code=400,
            content={"success": False, "message": "Cannot start splat capture while recording"}
        )
    if status["state"] == "splat_capture":
        return JSONResponse(
            status_code=400,
            content={"success": False, "message": "Splat capture already in progress"}
        )

    if sendCommand("splat_start"):
        # Wait briefly for capture service to acknowledge
        await asyncio.sleep(1)
        return {"success": True, "message": "Splat capture started"}
    return JSONResponse(
        status_code=500,
        content={"success": False, "message": "Failed to start splat capture"}
    )


@app.post("/api/splat/stop")
async def stopSplatCapture():
    """Stop Gaussian Splat capture and create ZIP"""
    if sendCommand("splat_stop"):
        await asyncio.sleep(1)
        return {"success": True, "message": "Splat capture stopping, creating ZIP..."}
    return JSONResponse(
        status_code=500,
        content={"success": False, "message": "Failed to stop splat capture"}
    )


@app.get("/api/splat/status")
async def getSplatStatus():
    """Get current Gaussian Splat capture status"""
    try:
        if os.path.exists(SPLAT_STATUS_FILE):
            with open(SPLAT_STATUS_FILE, 'r') as f:
                data = json.load(f)
            # If complete but ZIP was deleted, return idle
            if data.get("state") == "complete" and data.get("zipFile"):
                zipPath = os.path.join(VIDEO_DIR, data["zipFile"])
                if not os.path.exists(zipPath):
                    return {"state": "idle", "count": 0, "zipReady": False, "zipFile": ""}
            return data
    except (json.JSONDecodeError, IOError) as e:
        print(f"Error reading splat status: {e}")
    return {"state": "idle", "count": 0, "zipReady": False, "zipFile": ""}


@app.get("/api/splat/download/{filename}")
async def downloadSplatZip(filename: str):
    """Download a Gaussian Splat ZIP file"""
    # Security: prevent path traversal
...
@app.delete("/api/splat/{filename}")
async def deleteSplatZip(filename: str):
    """Delete a Gaussian Splat ZIP file"""
    if ".." in filename or filename.startswith("/") or not filename.endswith(".zip"):
        raise HTTPException(status_code=400, detail="Invalid filename")
    ...
    try:
        os.remove(filepath)
        # Reset splat status if the deleted file was the current session's ZIP
        try:
            if os.path.exists(SPLAT_STATUS_FILE):
                with open(SPLAT_STATUS_FILE, 'r') as f:
                    splatData = json.load(f)
                if splatData.get("zipFile") == filename:
...
@app.get("/api/splat/sessions")
async def listSplatSessions():
    """List available Gaussian Splat ZIP files"""
    sessions = []
    try:
        for f in sorted(Path(VIDEO_DIR).iterdir(), key=lambda x: x.stat().st_mtime, reverse=True):
            if f.is_file() and f.name.startswith("splat_") and f.name.endswith(".zip"):
                stat = f.stat()
                sessions.append({
                    "name": f.name,
                    "size": stat.st_size,
        ...
        print(f"Error listing splat sessions: {e}")
