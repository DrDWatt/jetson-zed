...
class CaptureService:
    def __init__(self):
        ...
        # Gaussian Splat capture state
        self.splatCapturing = False
        self.splatThread = None
        self.splatStopEvent = threading.Event()
        self.splatSessionDir = None
        self.splatCount = 0
    ...
    def initCamera(self):
        ...
        if ZED_AVAILABLE:
            try:
                ...
                depthModeStr = self.settings.get("depthMode", "NEURAL")
                if depthModeStr == "NEURAL":
                    self.updateStatus("initializing",
                        "Opening camera with NEURAL depth (engine may be optimizing, please wait...)")
                else:
                    self.updateStatus("initializing",
                        "Opening camera ({} depth)...".format(depthModeStr))
                ...
                if err == sl.ERROR_CODE.SUCCESS:
                    ...
                    # Update mtime so handleSettingsChange doesn't re-trigger
                    try:
                        self.settingsMtime = os.path.getmtime(SETTINGS_FILE)
                    except Exception:
                        pass
                    return True
                else:
    ...
    def handleSettingsChange(self):
        ...
        try:
            ...
            if self.recording or self.splatCapturing:
                print("Cannot change settings while recording/capturing")
                return
    ...
    # --- Gaussian Splat Still Capture ---

    def startSplatCapture(self):
        """Start capturing stills at current resolution/depth for Gaussian splatting"""
        if self.recording:
            print("Cannot start splat capture while recording")
            return False
        if self.splatCapturing:
            print("Splat capture already in progress")
            return False
        if not self.zed:
            print("Camera not available for splat capture")
            return False

        self.splatCapturing = True
        self.splatStopEvent.clear()
        self.splatCount = 0

        # Create session directory with timestamp
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.splatSessionDir = os.path.join(SPLAT_OUTPUT_DIR, "splat_{}".format(timestamp))
        Path(self.splatSessionDir).mkdir(parents=True, exist_ok=True)

        res = self.settings.get("resolution", "HD2K")
        depth = self.settings.get("depthMode", "NEURAL")
        self.updateSplatStatus("capturing", 0, self.splatSessionDir)
        self.updateStatus("splat_capture", "Capturing {} {} stills".format(res, depth))

        self.splatThread = threading.Thread(target=self.splatCaptureLoop)
        self.splatThread.start()
        print("Splat capture started ({} {}): {}".format(res, depth, self.splatSessionDir))
        return True

    def stopSplatCapture(self):
        """Stop splat capture and create ZIP archive"""
        if not self.splatCapturing:
            print("No splat capture in progress")
            return False

        self.splatStopEvent.set()
        if self.splatThread:
            self.splatThread.join(timeout=10)

        print("Splat capture stopped: {} images".format(self.splatCount))
        self.updateStatus("idle", "Splat capture complete: {} images".format(self.splatCount))

        # Create ZIP in background
        if self.splatSessionDir and self.splatCount > 0:
            zipThread = threading.Thread(target=self.createSplatZip, args=(self.splatSessionDir,))
            zipThread.start()
        else:
            self.updateSplatStatus("complete", self.splatCount, self.splatSessionDir, zipReady=False)

        self.splatCapturing = False
        return True

    def _getSensorPayload(self):
        """Capture synchronized sensor data and return as a dictionary"""
        sensorsData = sl.SensorsData()
        self.zed.get_sensors_data(sensorsData, sl.TIME_REFERENCE.IMAGE)

        imu = sensorsData.get_imu_data()
        baro = sensorsData.get_barometer_data()
        mag = sensorsData.get_magnetometer_data()
        temp = sensorsData.get_temperature_data()

        payload = {
            "imu": {
                "acceleration": list(imu.get_linear_acceleration()),
                "gyroscope": list(imu.get_angular_velocity()),
                "orientation": list(imu.get_orientation().get())
            },
            "environment": {
                "pressure_hpa": baro.pressure if baro.is_available else None,
                "temperature_c": None
            },
            "magnetometer": list(mag.get_magnetic_field_calibrated()) if mag.is_available else None
        }

        # Get IMU temperature (available on ZED 2/2i)
        imuTemp = sl.SENSOR_LOCATION.IMU if hasattr(sl, 'SENSOR_LOCATION') else None
        try:
            tempVal = temp.get(sl.SENSOR_LOCATION.IMU)
            payload["environment"]["temperature_c"] = tempVal if tempVal != 0.0 else None
        except Exception:
            pass

        return payload

    def splatCaptureLoop(self):
        """Capture loop: grab full-resolution frames with RGB, depth PFM, and sensor JSON"""
        image = sl.Mat()
        depthMap = sl.Mat()

        while not self.splatStopEvent.is_set() and self.splatCount < SPLAT_MAX_IMAGES:
            # Grab a frame
            err = self.zed.grab(self.runtimeParams)
            if err != sl.ERROR_CODE.SUCCESS:
                time.sleep(0.1)
            ...
            # Get image timestamp
            timestampNs = self.zed.get_timestamp(sl.TIME_REFERENCE.IMAGE).get_nanoseconds()

            # Retrieve full-resolution left image (BGRA)
            self.zed.retrieve_image(image, sl.VIEW.LEFT)
            frame = image.get_data()
            if frame is None:
                time.sleep(0.1)
            ...
            # Retrieve raw 32-bit depth map
            self.zed.retrieve_measure(depthMap, sl.MEASURE.DEPTH)

            self.splatCount += 1
            baseName = "frame_{:04d}".format(self.splatCount)
            imgPath = os.path.join(self.splatSessionDir, baseName + ".png")
            pfmPath = os.path.join(self.splatSessionDir, baseName + ".pfm")
            jsonPath = os.path.join(self.splatSessionDir, baseName + ".json")

            # Save RGB as lossless PNG
            try:
                rgb = frame[:, :, [2, 1, 0]]
                img = Image.fromarray(rgb)
                img.save(imgPath, "PNG")
            except Exception as e:
                print("Error saving splat frame {}: {}".format(self.splatCount, e))

            # Save depth as PFM (raw 32-bit float, metres per pixel)
            try:
                pfmErr = depthMap.write(pfmPath)
                if pfmErr != sl.ERROR_CODE.SUCCESS:
                    print("Error writing PFM frame {}: {}".format(self.splatCount, pfmErr))
            except Exception as e:
                print("Error saving depth PFM {}: {}".format(self.splatCount, e))

            # Save sensor sidecar JSON (IMU, barometer, magnetometer)
            try:
                sensorPayload = self._getSensorPayload()
                sensorPayload["image_file"] = baseName + ".png"
                sensorPayload["depth_file"] = baseName + ".pfm"
                sensorPayload["frame_number"] = self.splatCount
                sensorPayload["timestamp_ns"] = timestampNs
                with open(jsonPath, 'w') as f:
                    json.dump(sensorPayload, f, indent=2)
            except Exception as e:
                print("Error saving sensor data for frame {}: {}".format(self.splatCount, e))

            # Update status
            self.updateSplatStatus("capturing", self.splatCount, self.splatSessionDir)
            if self.splatCount % 10 == 0:
                print("Splat capture: {} images taken".format(self.splatCount))
    ...
    def updateSplatStatus(self, state, count, sessionDir, zipReady=False, zipFile=None):
        """Write splat capture status for web server to read"""
        status = {
            "state": state,
            "count": count,
            "sessionDir": os.path.basename(sessionDir) if sessionDir else "",
            "zipReady": zipReady,
            "zipFile": zipFile or "",
            "timestamp": datetime.now().isoformat()
        }
        try:
            with open(SPLAT_STATUS_FILE, 'w') as f:
                json.dump(status, f, indent=2)
        except Exception as e:
            print("Error writing splat status: {}".format(e))

    def createSplatZip(self, sessionDir):
        """Create ZIP archive of captured stills"""
        self.updateSplatStatus("zipping", self.splatCount, sessionDir)
        self.updateStatus("idle", "Creating ZIP archive...")
        sessionName = os.path.basename(sessionDir)
        zipPath = os.path.join(VIDEO_OUTPUT_DIR, "{}.zip".format(sessionName))

        try:
            print("Creating ZIP: {}".format(zipPath))
            with zipfile.ZipFile(zipPath, 'w', zipfile.ZIP_DEFLATED) as zf:
                for root, dirs, files in os.walk(sessionDir):
                    for file in sorted(files):
                        filePath = os.path.join(root, file)
                        arcName = os.path.join(sessionName, file)
                        zf.write(filePath, arcName)

            zipSize = os.path.getsize(zipPath)
            sizeMb = zipSize / (1024 * 1024)
            print("ZIP created: {} ({:.1f} MB)".format(zipPath, sizeMb))

            # Clean up individual images to save space
            shutil.rmtree(sessionDir, ignore_errors=True)

            self.updateSplatStatus("complete", self.splatCount, sessionDir,
                                   zipReady=True, zipFile=os.path.basename(zipPath))
            self.updateStatus("idle", "Splat ZIP ready: {:.1f} MB".format(sizeMb))
        except Exception as e:
            print("Error creating splat ZIP: {}".format(e))
            self.updateSplatStatus("error", self.splatCount, sessionDir)
            self.updateStatus("idle", "Splat ZIP failed: {}".format(e))
    ...
    def processCommand(self, command):
        ...
        elif command == "splat_start":
            return self.startSplatCapture()
        elif command == "splat_stop":
            return self.stopSplatCapture()
        elif command == "status":
            return True
        else:
    ...
    def run(self):
        ...
        # Clear stale control file from previous run
        try:
            if os.path.exists(CONTROL_FILE):
                with open(CONTROL_FILE, 'w') as f:
                    f.write("")
                print("Cleared stale control file")
        except Exception:
            pass

        # Reset stale splat status if it was mid-capture when we stopped
        try:
            splatStatusPath = os.path.join(VIDEO_OUTPUT_DIR, ".splat_status")
            if os.path.exists(splatStatusPath):
                with open(splatStatusPath, 'r') as f:
                    splatData = json.load(f)
                if splatData.get("state") in ("capturing", "zipping"):
                    print("Resetting stale splat status from '{}'".format(splatData.get("state")))
                    with open(splatStatusPath, 'w') as f:
                        json.dump({"state": "idle", "count": 0, "zipReady": False, "zipFile": ""}, f)
        except Exception:
            pass
        ...
        self.updateStatus("idle", "Camera ready")
        ...
        try:
            while True:
                ...
                # Grab live frame when idle (~15fps)
                if not self.recording and not self.splatCapturing and self.zed:
                    if self.zed.grab(self.runtimeParams) == sl.ERROR_CODE.SUCCESS:
                        self._writeLiveFrame()
